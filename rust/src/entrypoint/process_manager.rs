//! Process management for InfiniEntrypoint
...
impl ProcessManager {
    ...
    fn build_mock_command(&self) -> Result<Command, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if let Some(endpoints) = &self.state.config.etcd_endpoints {
            cmd.arg("--etcd-endpoints").arg(endpoints);
        }
        if let Some(prefix) = &self.state.config.discovery_prefix {
            cmd.arg("--discovery-prefix").arg(prefix);
        }
