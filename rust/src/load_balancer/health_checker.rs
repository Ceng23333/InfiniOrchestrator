...
impl HealthChecker {
    ...
    /// Perform health check on a service instance using entrypoint URL
    pub async fn check_health(&self, service: &ServiceInstance) -> bool {
        let check_url = format!("{}/health", service.entrypoint_url);
        ...
        match self.client.get(&check_url).send().await {
            ...
            Err(e) => {
                warn!(
                    "Health check failed for service {} (entrypoint: {}): {}",
                    service.name, service.entrypoint_url, e
                );
                service.set_healthy(false).await;
                let mut error_count = service.error_count.write().await;
                *error_count += 1;
