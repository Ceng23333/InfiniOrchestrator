...
if ss -lntp 2>/dev/null | grep -q ":${PORT} "; then
  ...
  echo "  A Distribution Frontend may already serve /panel (e.g. qwen opt20260811 on 8800)." >&2
  echo "  Reuse that Frontend, or: FRONTEND_HOST_PORT=18880 $0" >&2
  echo "  For Prom/Grafana only: PROM_CONFIG=prometheus-host.yml ./compose.sh --profile observability up -d" >&2
  exit 98
fi
