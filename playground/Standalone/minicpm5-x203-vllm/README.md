...
Case id: `minicpm5-x203-vllm` (`case.toml`).

## Quickstart

```bash
cd InfiniOrchestrator/playground/Standalone/minicpm5-x203-vllm

ln -sfn minicpm5.16a3.v0314 /root/zenghua/models/minicpm5
./build-wrap-image.sh
./run-wrap.sh

curl -s "http://$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' minicpm5-x203-vllm):18180/v1/models"
```
...
export CASE_ID=minicpm5-x203-vllm
export CASE_PATH="${IO_ROOT}/playground/Standalone/minicpm5-x203-vllm/case.toml"
export BENCH_BACKEND=vllm
export DEV_CONTAINER_NAME=minicpm5-x203-vllm
export DEV_PORT=18180
export BENCH_TARGET_URL=http://$(docker inspect -f '{{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' minicpm5-x203-vllm):18180
