...
# Hygiene: drop dest-only processors that InfiniTensor main folded into
# sentencepiece_processor.py (no-op on a clean Phase 1 dest).
_proc_src="${SOURCE_ROOT}/InfiniLM/python/infinilm/processors"
_proc_dst="/workspace/InfiniLM/python/infinilm/processors"
if [[ -d "${_proc_src}" ]]; then
  for _f in internlm_processor.py mistral_processor.py; do
    if [[ ! -f "${_proc_src}/${_f}" ]]; then
      docker exec "${CONTAINER_NAME}" rm -f "${_proc_dst}/${_f}"
    fi
  done
fi
...
CASE_ID=${CASE_ID:-9g_8b_thinking-x203-inf--main}
BE_ABBR=inf
...
echo ""
echo "Next: cd ../docker-compose && cp .env.frontend.example .env"
