...
# Never overwrite the locked --deploy runtime-base.
DEPLOY_RUNTIME_BASE_TAG="${DEPLOY_RUNTIME_BASE_TAG:-infini-orchestrator-metax:metax-hpcc-ai370-runtime-base-20260813}"
...
XMAKE_SEED_DIR="${XMAKE_SEED_DIR:-${MONOREPO_ROOT}/.xmake_seed}"
PHASE1_GPU="${PHASE1_GPU:-1}"
# Read-only source of InfiniCore third_party/spdlog (pin snapshots often omit submodules).
# Never commit/retag this image.
...
if [[ "${RUNTIME_BASE_TAG}" == "${DEPLOY_RUNTIME_BASE_TAG}" ]]; then
  ...
  echo "  set RUNTIME_BASE_TAG to a unique tag (e.g. ...-runtime-base-main-${BUILD_TS})" >&2
  exit 1
fi
...
IO_SHA="$(git -C "${IO_ROOT}" rev-parse --short HEAD 2>/dev/null || echo unknown)"
# Pin snapshots (itw-pins) often have no usable .git; prefer SHA files / caller pins.
if [[ -f "${SOURCE_ROOT}/InfiniLM.sha" ]]; then
  IL_SHA="$(tr -d '[:space:]' < "${SOURCE_ROOT}/InfiniLM.sha")"
else
  IL_SHA="${IL_SHA_PIN:-$(git -C "${SOURCE_ROOT}/InfiniLM" rev-parse HEAD 2>/dev/null || echo unknown)}"
fi
if [[ -f "${SOURCE_ROOT}/InfiniCore.sha" ]]; then
  IC_SHA="$(tr -d '[:space:]' < "${SOURCE_ROOT}/InfiniCore.sha")"
else
  IC_SHA="${IC_SHA_PIN:-$(git -C "${SOURCE_ROOT}/InfiniCore" rev-parse HEAD 2>/dev/null || echo unknown)}"
fi
if [[ -f "${SOURCE_ROOT}/ITW_TAG" ]]; then
  ITW_TAG="$(tr -d '[:space:]' < "${SOURCE_ROOT}/ITW_TAG")"
else
...
# Vendor BASE has no xmake. Copy the binary + program files + package cache
# from infinilm-dev-hpcc37 (or host). Do NOT seed from deploy runtime-base
# (that image also lacks xmake; overwriting its tag is forbidden).
seed_xmake() {
  ...
  if [[ ! -d "${_xstage}/share" || -z "$(ls -A "${_xstage}/share" 2>/dev/null || true)" ]]; then
    if [[ -d /root/.local/share/xmake ]]; then
      echo "    program files from host /root/.local/share/xmake"
      mkdir -p "${_xstage}/share"
      cp -a /root/.local/share/xmake/. "${_xstage}/share/"
    fi
  fi
  if [[ ! -d "${_xstage}/dotxmake" || -z "$(ls -A "${_xstage}/dotxmake" 2>/dev/null || true)" ]]; then
    if [[ -d "${XMAKE_SEED_DIR}" ]]; then
      echo "    ~/.xmake cache from ${XMAKE_SEED_DIR}"
      mkdir -p "${_xstage}/dotxmake"
      cp -a "${XMAKE_SEED_DIR}/." "${_xstage}/dotxmake/"
    fi
  fi
...
# Pin snapshots often have empty git-submodule dirs. Copy spdlog/json headers from
# the locked deploy runtime-base (read-only; never overwrite that tag).
seed_infinicore_third_party() {
  ...
  echo "  Seeding InfiniCore/InfiniLM third_party headers from ${THIRD_PARTY_SEED_IMAGE} (read-only)..."
  local _seed _stage
  ...
  if [[ -f "${_stage}/spdlog/include/spdlog/spdlog.h" ]]; then
    ...
    echo "warning: spdlog not found in ${THIRD_PARTY_SEED_IMAGE}" >&2
  fi
  if [[ -d "${_stage}/json/single_include" ]]; then
    ...
    echo "warning: InfiniLM third_party/json not found in ${THIRD_PARTY_SEED_IMAGE}" >&2
  fi
  rm -rf "${_stage}"
}
