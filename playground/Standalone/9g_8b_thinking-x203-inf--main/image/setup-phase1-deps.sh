...
# When true, remove installed control-plane bins so cargo install runs again.
FORCE_RUST_REINSTALL="${FORCE_RUST_REINSTALL:-0}"
...
if [[ "${SKIP_RUST}" != "true" ]]; then
  if [[ "${FORCE_RUST_REINSTALL}" == "1" || "${FORCE_RUST_REINSTALL}" == "true" ]]; then
    echo "[phase1] FORCE_RUST_REINSTALL: removing /usr/local/bin/infini-{entrypoint,loadbalancer,babysitter,router}"
    rm -f /usr/local/bin/infini-entrypoint /usr/local/bin/infini-loadbalancer \
      /usr/local/bin/infini-babysitter /usr/local/bin/infini-router
  fi
  if [[ -x /usr/local/bin/infini-entrypoint && -x /usr/local/bin/infini-loadbalancer ]]; then
    echo "[phase1] InfiniEntrypoint binaries already present; skipping cargo install"
    command -v infini-entrypoint
    command -v infini-loadbalancer
  elif [[ -f ./scripts/install.sh ]]; then
    echo "[phase1] Installing control-plane via /app/scripts/install.sh..."
    ./scripts/install.sh \
    ...
    echo "[phase1] warning: InfiniEntrypoint bins missing and no /app/scripts/install.sh" >&2
  fi
fi
...
if [[ "${SKIP_INFINICORE_INFINILM}" != "true" ]]; then
  ...
  if [[ "${FORCE_XMAKE_BUILD:-false}" == "true" || "${_has_prebuilt}" != "true" ]]; then
    ...
    # InfiniTensor main xmake.lua uses python.module; HPCC-seeded xmake 2.9.9
    # only ships python.library (same rule InfiniCore uses for _infinicore).
...
chmod +x /app/docker_entrypoint.sh 2>/dev/null || true
...
if command -v pip >/dev/null 2>&1; then
  pip install -q janus 2>/dev/null || \
    ...
    echo "[phase1] warning: could not pip install janus" >&2
fi

echo "[phase1] Gate smoke checks..."
if command -v infini-entrypoint >/dev/null 2>&1; then
  command -v infini-entrypoint
  command -v infini-loadbalancer
  command -v infini-babysitter || true
else
  echo "[phase1] warning: InfiniEntrypoint binaries not on PATH yet" >&2
fi
# shellcheck source=/dev/null
source /app/env-set.sh
