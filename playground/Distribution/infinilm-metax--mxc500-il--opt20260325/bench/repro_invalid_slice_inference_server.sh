...
PAYLOAD="${REPO_SVC}/InfiniOrchestrator/deploy/cases/infinilm-metax-deployment-opt-20260325/results/repro-slice-payload.json"
MODEL="${MODEL:-/data-aisoft/zenghua/models/9g_8b_thinking_llama}"
PORT="${PORT:-18080}"
LOG="${LOG:-${REPO_SVC}/InfiniOrchestrator/deploy/cases/infinilm-metax-deployment-opt-20260325/results/repro-inference-server-$(date -u +%Y%m%d-%H%M%SZ).log}"
