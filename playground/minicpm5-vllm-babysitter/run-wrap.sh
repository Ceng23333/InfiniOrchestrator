#!/usr/bin/env bash
# Run wrap container: infini-babysitter → stock vLLM + minicpm5 plugin.
set -euo pipefail
...
CONFIG_IN_CONTAINER="/workspace/InfiniOrchestrator/playground/minicpm5-vllm-babysitter/config/master-minicpm5-vllm.toml"
...
docker rm -f 9g-vllm-babysitter >/dev/null 2>&1 || true
...
docker run -d \
  ...
  --entrypoint /bin/bash \
  "${IMAGE_TAG}" \
  -lc 'exec infini-babysitter --config-file "${BABYSITTER_CONFIGS}"'
