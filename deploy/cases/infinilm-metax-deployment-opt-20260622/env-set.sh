#!/usr/bin/env bash
# HPCC ai3.1.0.7 / torch2.6 runtime + build env for infinilm-metax-deployment-opt-20260622.
# Sourced by setup-in-container.sh and available at /app/env-set.sh in the committed image.
...
export LD_LIBRARY_PATH="${TORCH_LIB:-}:${INFINI_ROOT}/lib:${LD_LIBRARY_PATH:-}"
...
export PATH="${INFINI_ROOT}/bin:${HOME}/.cargo/bin:/usr/local/bin:${PATH}"
