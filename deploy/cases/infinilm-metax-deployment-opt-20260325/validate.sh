...
usage() {
  ...
  echo ""
  echo "SLAVE_PRESET (when SLAVE_IP given): fla (default), 2static(compat), 1static1vllm, 3vllm, or 1paged2vllm"
...
SLAVE_IP="${2:-}"
SLAVE_PRESET="${3:-${SLAVE_PRESET:-fla}}"
...
echo "=========================================="
echo "InfiniOrchestrator infinilm-metax-deployment-opt Validation"
echo "=========================================="
...
if [ -n "${SLAVE_IP}" ]; then
  case "${SLAVE_PRESET}" in
    fla)
      expected_services+=("slave-fla-9g-server" "slave-fla-qwen-paged-server")
      ;;
    2static)
      expected_services+=("slave-fla-9g-server" "slave-fla-qwen-paged-server")
      ;;
    ...
    1paged2vllm)
      ...
      ;;
    *)
      echo "  Warning: Unknown SLAVE_PRESET '${SLAVE_PRESET}'; expecting fla, 2static, 1static1vllm, 3vllm, or 1paged2vllm"
      ;;
  esac
fi
