#!/usr/bin/env bash
# Run unexpected-behavior fault-injection scenarios against a live inference endpoint.
# Per-step emits to raw/<platform>/<bench_id>/<date>/ via bench_harness.emit.
...
# shellcheck disable=SC1091
source "${HARNESS_ROOT}/unexpected_behavior/lib/common.sh"
# shellcheck disable=SC1091
...
DEFAULT_SCENARIOS="happy_single,cancel_mid_decode,client_disconnect_stream,curl_timeout_burst,concurrent_mixed"
IFS=',' read -r -a SCENARIO_LIST <<< "${SCENARIOS:-${DEFAULT_SCENARIOS}}"
...
for scenario in "${SCENARIO_LIST[@]}"; do
  scenario="${scenario// /}"
  script="${HARNESS_ROOT}/unexpected_behavior/scenarios/${scenario}.sh"
