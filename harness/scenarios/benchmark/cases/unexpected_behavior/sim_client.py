...
def _build_payload(model: str, prompt: str, max_tokens: int, stream: bool) -> bytes:
    return json.dumps(
        chat_completions_payload(model, prompt, max_tokens=max_tokens, stream=stream)
    ).encode("utf-8")


def _post(url: str, data: bytes, timeout: float | None):
    req = chat_completions_request(url, data)
    return req, timeout
...
async def cmd_cancel_stream(args: argparse.Namespace) -> int:
    ...
    async def _stream_and_cancel() -> None:
        ...
        def _open():
            req = chat_completions_request(args.base_url, data)
